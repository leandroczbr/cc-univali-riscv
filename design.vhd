-- =============================================================================
--  design.vhd  --  Topo do Processador RISC-V 32-bit Monociclo
-- =============================================================================
--
--  Responsabilidade deste módulo:
--    Interligar (fazer o "fiação"/wiring de) todos os blocos do datapath:
--      - controle       : decodifica a instrução e gera os sinais de controle
--      - mux5 (MUX_REG_DST)  : escolhe qual campo da instrução é o registrador
--                               de destino
--      - regmemory      : banco de registradores (leitura/escrita)
--      - mux32 (MUX_FONTE_ULA) : escolhe se o 2º operando da ULA vem do
--                               registrador ou do imediato
--      - ulaop          : traduz funct3 + bit30 no código de operação da ULA
--      - ula            : executa a operação aritmética/lógica
--
--  Observação sobre mux5/mux32:
--    As entidades "mux5" e "mux32" (multiplexadores de 5 e 32 bits) não
--    fazem parte dos arquivos fornecidos para este refactor, então as
--    portas usadas na instanciação delas (a, b, sel, o) foram mantidas
--    exatamente como estavam, já que não temos acesso ao código-fonte
--    dessas entidades para renomear suas portas com segurança.
--
--  Observação sobre as portas de teste:
--    As portas valor_escrita_teste, endereco_escrita_teste e
--    habilita_escrita_teste existem na interface da entidade, mas (assim
--    como no arquivo original) não estão conectadas a nenhum sinal interno
--    dentro desta arquitetura. Ou seja, elas continuam sem efeito sobre o
--    funcionamento do processador — esse comportamento foi mantido
--    propositalmente, pois a tarefa é apenas renomear/comentar, não alterar
--    a lógica.
-- =============================================================================
...
begin
    ...
    DECODIFICADOR_ULAOP: entity work.ulaop
        port map (
            ...
            bit30_da_instrucao  => instrucao(30),
